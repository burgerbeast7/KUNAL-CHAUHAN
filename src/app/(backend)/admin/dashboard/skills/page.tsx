  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold font-orbitron text-white">Skills</h1>
        <button
        ...
        {skills.map((s) => (
          <div key={s._id} className="bg-zinc-900 border border-white/10 p-6 rounded-xl relative group">
            <div className="absolute top-4 right-4 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => openEditModal(s)} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg text-white"><Edit2 size={16}/></button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-zinc-900 border border-white/10 p-8 rounded-2xl w-full max-w-lg relative text-white">
            ...
            <h2 className="text-2xl font-bold mb-6 font-orbitron">{editingId ? "Edit Skill" : "Add Skill"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input required type="text" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full px-4 py-2 bg-black border border-white/10 rounded-lg outline-none focus:border-white/50 text-white" placeholder="e.g. Frontend, Backend, Tools" />
              </div>
              <div>
